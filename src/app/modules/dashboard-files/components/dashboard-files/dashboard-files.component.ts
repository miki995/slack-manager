...
import { EFilesCount, EFilesFilter, IFile, IFilesQueryParams, IFilesResponse } from '../../../../models/file-filter';
...
export class DashboardFilesComponent implements OnInit {
  ...
  allFilesResponse$: Observable<IFilesResponse>;
  ...
  ngOnInit(): void {
    ...
    this.allFilesResponse$ = this.store.pipe(select(getDashboardState), pluck('allFilesResponse'), distinctUntilChanged<IFilesResponse>());
  ...
  startBulkDeleteAll(files: IFile[]): void {
    ...
    this.store.dispatch({ type: dashboardActions.DASHBOARD_SET_BULK_DELETE_ALL });

    this.deleteMultipleFiles(files.map(file => file.id), true);
  }
  ...
  deleteMultipleFiles(files: string[], bulk?: boolean): void {
    let interval = 1000;
